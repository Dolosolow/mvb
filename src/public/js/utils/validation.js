      ...
      switch($(elm).attr('name')) {
        ...
        case 'password':
          ...
          if(!validator.matches($(elm).val(), passwordRegex)) {
            validationError(elm, `Password should contain atleast one uppercase letter, one number, and one symbol.`);
        ...
        case 'confirmPassword':
          ...
          if($(elm).val() !== password) {
            validationError(elm, `Values entered in 'Password' and 'Confirm Password' don't match.`);
          } else {
            validationSuccess(elm);
          }
        default: 
